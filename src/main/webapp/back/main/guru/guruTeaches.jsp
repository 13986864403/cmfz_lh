...
<script>
    $(function(){
        $("#tbGuruTeachs").datagrid({
            url:'${pageContext.request.contextPath}/article/findArticleByPage?difference='+"上师言教",//用来请求远程数据
            ...
            columns:[[
                ...
                {title:'操作',field:'options',width:120,
                    formatter:function(value,row,index){
                        return "<a  class='options' onclick=\"delOneGuruTeaches('"+row.id+"')\" data-options=\"iconCls:'icon-delete',plain:true\">删除</a>&nbsp;&nbsp;" +
                            "<a class='options' onclick=\"updateGuruTeaches('"+row.id+"','"+row.shows+"')\" data-options=\"iconCls:'icon-edit',plain:true\">修改</a>";
    ...
    //保存用户
    function addGuruTeaches(){
        $("#addGuruTeachesDialog").dialog({
            buttons:[{
                ...
                handler:function(){
                    $("#addGuruTeachesInput").form('submit',{
                        ...
                        success:function(result){
                            ...
                            //保存成功关闭对话框
                            $("#addGuruTeachesDialog").dialog('close');
                            //刷新页面
                            $("#tbGuruTeachs").datagrid('reload');
                        }
                    })
    ...
    //修改文章
    function updateGuruTeaches(id,shows){
                ...
                handler:function(){
                    $("#updateGuruTeachesInput").form('submit',{
                        ...
                        success:function (result) {//注意一定是json字符串  使用需要转为js对象
                            ...
                            //刷新datagrid
                            $("#tbGuruTeachs").datagrid('reload');//刷新当前datagrid
                        }
                    })
                }
            },{
    ...
    //批量删除
    function delGuruTeaches() {
        var rows = $("#tbGuruTeachs").datagrid('getSelections');
        if(rows.length<=0){
...
<%--表格--%>
<table id="tbGuruTeachs"></table>

<%--工具栏--%>
<div id="tbarGuruTeaches">
    <a href="javascript:;" onclick="addGuruTeaches();" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">上传文章</a>
    <a href="javascript:;" onclick="delGuruTeaches();" class="easyui-linkbutton" data-options="iconCls:'icon-delete',plain:true">批量删除</a>
</div>

<div id="addGuruTeachesDialog" data-options="href:'${pageContext.request.contextPath}/back/main/guru/addGuruTeaches.jsp',iconCls:'icon-table_save',width:600,height:400,title:'上传文章'"></div>

<div id="updateGuruTeachesDialog" data-options="iconCls:'icon-edit',width:600,height:400,title:'修改文章信息'"></div>
